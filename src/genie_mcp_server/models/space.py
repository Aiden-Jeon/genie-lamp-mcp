...
class GenieSpaceInstruction(BaseModel):
    ...
    priority: Optional[int] = Field(
        None, description="Priority of the instruction (higher = more important)"
    )
...
class GenieSpaceSQLExpression(BaseModel):
    ...
    instruction: Optional[str] = Field(
        None, description="Guidance on when and how to use this expression"
    )
...
class GenieSpaceSQLMeasure(BaseModel):
    ...
    sql: str = Field(
        ..., description="SQL aggregation expression (e.g., 'SUM(orders.order_amount)')"
    )
    ...
    instruction: Optional[str] = Field(
        None, description="Guidance on when and how to use this measure"
    )
...
class GenieSpaceSQLSnippets(BaseModel):
    ...
    filters: list[GenieSpaceSQLFilter] = Field(
        default_factory=list, description="SQL filters (WHERE conditions)"
    )
    expressions: list[GenieSpaceSQLExpression] = Field(
        default_factory=list, description="SQL expressions (dimensions/calculated fields)"
    )
    measures: list[GenieSpaceSQLMeasure] = Field(
        default_factory=list, description="SQL measures (aggregations)"
    )
...
class GenieSpaceJoinSpec(BaseModel):
    ...
    left_table: str = Field(
        ..., description="Left table in fully qualified format: catalog.schema.table"
    )
    right_table: str = Field(
        ..., description="Right table in fully qualified format: catalog.schema.table"
    )
    join_type: str = Field(..., description="Join type: INNER, LEFT, RIGHT, or FULL")
    join_condition: str = Field(
        ..., description="Join condition (e.g., 'left_table.id = right_table.foreign_id')"
    )
    description: Optional[str] = Field(
        None, description="Explanation of the relationship between tables"
    )
    instruction: Optional[str] = Field(
        None, description="Guidance on when and how to use this join"
    )
...
class GenieSpaceConfig(BaseModel):
    ...
    # Data configuration
    tables: list[GenieSpaceTable] = Field(
        default_factory=list, description="Tables to include in the space"
    )
    join_specifications: list[GenieSpaceJoinSpec] = Field(
        default_factory=list, description="Explicit join relationships between tables"
    )

    # Instructions and examples
    instructions: list[GenieSpaceInstruction] = Field(
        default_factory=list, description="Plain text instructions"
    )
    example_sql_queries: list[GenieSpaceExampleSQL] = Field(
        default_factory=list, description="Example SQL queries"
    )
    sql_snippets: Optional[GenieSpaceSQLSnippets] = Field(
        None, description="SQL snippets (filters, expressions, measures)"
    )

    # Testing
    benchmark_questions: list[GenieSpaceBenchmark] = Field(
        default_factory=list, description="Benchmark questions for testing"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "benchmark_questions": [{"question": "What were the top 10 selling products last week?"}],
            }
        }
    )
...
class LLMResponse(BaseModel):
    ...
    genie_space_config: GenieSpaceConfig = Field(
        ..., description="The generated Genie space configuration"
    )
    reasoning: Optional[str] = Field(
        None, description="LLM's reasoning for the configuration choices"
    )
    confidence_score: Optional[float] = Field(
        None, description="Confidence score (0-1) for the configuration"
    )


class ValidationReport(BaseModel):
    """Validation report for a Genie space configuration."""

    valid: bool = Field(..., description="Whether the configuration is valid")
    errors: list[str] = Field(default_factory=list, description="List of validation errors")
    warnings: list[str] = Field(default_factory=list, description="List of validation warnings")
    score: int = Field(
        ..., description="Quality score (0-100) based on completeness and best practices"
    )
